...
float turnSquare = 17.5; //22~45 do
float turn1 = 0;
float now = 0;
uint8_t left = 1;
uint8_t right = 1;
...
void dieu_khien()
{
	...
	else if (Num_CLK < 3000 && flagTurn == 0){

		dung();
		delay_1MS(2);
		phaiv2();
		Turn_motor_speed(turn, 1400);
		dung();
		now += turn;
		delay_1MS(2);
		count++;
	...
	else if (Num_CLK < 3000 && flagTurn == 1){
		dung();
		delay_1MS(2);
		traiv2();
		Turn_motor_speed(turn, 1400);
...
void setValue(){
	turn1 = turnSquare/4;
	now = turnSquare * 1;
	midStone = turnSquare * 2;
	leftStone = midStone - turnSquare;
	rightStone = midStone + turnSquare;
}

void quayXe(){
	if (Num_CLK <= 4000 && flagTurn == 1 && right == 1){
		dung();
		delay_1MS(1);
		phaiv2();
		Turn_motor_speed(turn1, 1400);
		now += turn1;
		delay(100);
		dung();
		delay(250);
	}
	else if (Num_CLK <= 4000 && flagTurn == 0 && left == 1){
		dung();
		delay_1MS(1);
		traiv2();
		Turn_motor_speed(turn1, 1400);
