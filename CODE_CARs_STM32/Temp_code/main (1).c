...
void Turn_motor_speed(uint16_t time,uint16_t  time_run ){  //ham Turn motor dieu khien toc do mem deo
	while(time){
	GPIOA->ODR |=(uint16_t)0x001;
	GPIOA->ODR |=(uint16_t)0x002;
	delay(time_run);
...
uint32_t  startup=1500;
uint32_t  RL_speed=800;
uint32_t  angle=70;

uint32_t  speed_up=250;

void dieu_khien(){
	if(Num_CLK <4000 && flashRL==0){
		Send_String("Trai->Phai \r\n");
		Motor_Enable();
		dung();
		delay(15);
		...
		trai();
//		delay_1MS(1);
		Turn_motor_speed(angle-13,RL_speed); //50 | 800
...
//		delay(startup); //1500
		...
		phai();
//		delay_1MS(1);
		Turn_motor_speed(angle,RL_speed);
...
//			delay_1MS(1);
			...
			Motor_Enable();
//			delay_1MS(1);
			Turn_motor_speed(angle-13,RL_speed);
